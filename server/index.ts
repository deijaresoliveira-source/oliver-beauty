...
async function startServer() {
  ...
  if (process.env.NODE_ENV === "production") {
    // Build de produção separado para Hostinger:
    // - Frontend Vite: dist/client
    // - Backend Express: dist/server
    // Quando compilado, __dirname fica em dist/server.
    ...
    const staticPath = possibleStaticPaths.find((candidate) => {
      try {
    ...
    if (!staticPath) {
      throw new Error(
        `Frontend build não encontrado. Pastas verificadas: ${possibleStaticPaths.join(", ")}`
      );
    }
